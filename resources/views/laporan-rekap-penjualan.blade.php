...
<head>
    ...
    <title>Halaman Laporan Rekap Penjualan</title>
    ...
    <main class="main-container">
        ...
        <div class="report-filter-wrapper">
            <button type="button" class="report-tab active-penjualan"
                data-url="{{ route('laporan.rekap.penjualan') }}">
                Penjualan
            </button>
            <button type="button" class="report-tab" data-url="{{ route('laporan.rekap.pembelian') }}">
                Pembelian
            </button>
        </div>
        ...
        <div class="table-container">
            ...
            <div class="pagination-wrapper">
                <div class="pagination-btns">
                    @if ($dataRekap->onFirstPage() || $dataRekap->isEmpty())
                        <button class="btn-page disabled" disabled>Sebelumnya</button>
                    @else
                        <a href="{{ $dataRekap->previousPageUrl() }}" class="btn-page">Sebelumnya</a>
                    @endif
                    @if ($dataRekap->hasMorePages())
                        <a href="{{ $dataRekap->nextPageUrl() }}" class="btn-page">Berikutnya</a>
                    @else
            ...
            <div style="text-align: center;">
                ...
                <form action="{{ route('laporan.rekap.hapus-semua') }}" method="POST" id="formHapusSemua">
                    ...
                    <div style="display: flex; justify-content: center; gap: 10px;">
                        <button type="button" class="btn-modal-cancel" id="btnBatalHapus"
                            style="flex: 1; padding: 12px 0; border: 1.5px solid #CBD5E1; transition: all 0.3s ease;">
                            Tidak
                        </button>
                        <button type="submit" class="btn-modal-confirm" style="flex: 1; padding: 12px 0;">
                            Iya
                        </button>
                    </div>
                </form>
    ...
    <script>
        ...
        if (filterForm) {
            filterForm.addEventListener('submit', function(e) {
                ...
                const valMulai = tglMulai ? tglMulai.value : "";
                const valAkhir = tglAkhir ? tglAkhir.value : "";
        ...
        document.querySelectorAll('.report-tab').forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                if (targetUrl && !this.classList.contains('active-penjualan') && !this.classList.contains(
                        'active-pembelian')) {
                    triggerLoading(targetUrl);
                }
            });
        });
