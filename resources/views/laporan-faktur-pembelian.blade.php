...
<head>
    ...
    <title>Halaman Laporan Faktur Pembelian</title>
        ...
        <div class="table-container">
            <div class="table-header-content">
                <div style="display: flex; align-items: center; gap: 15px;">
                    <h3 style="margin: 0; font-size: 18px; font-weight: 800; color: #0F172A;">Faktur Pembelian</h3>
                    <span class="date-badge">
                        Tanggal: {{ \Carbon\Carbon::parse($tanggal)->format('d/m/Y') }}
                    </span>
                </div>
                <a href="{{ route('laporan.rekap.pembelian') }}?page={{ $rekapPage }}&tgl_mulai={{ request('tgl_mulai') }}&tgl_akhir={{ request('tgl_akhir') }}"
                    class="btn-back" id="btnBackToRekap">
            ...
            <div class="faktur-table-scroll-wrapper">
                <table>
                    <thead>
                        <tr>
                            <th class="col-faktur-no">No</th>
                            <th class="col-faktur-id">No. Faktur</th>
                            <th class="col-faktur-pemasok">Pemasok</th>
                            <th class="col-faktur-total">Total Pembayaran</th>
                            <th class="col-faktur-metode">Metode Pembayaran</th>
                            <th class="col-faktur-aksi">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($transaksiPembelian as $item)
                            <tr>
                                <td class="col-faktur-no">
                                    {{ ($transaksiPembelian->currentPage() - 1) * $transaksiPembelian->perPage() + $loop->iteration }}
                                </td>
                                <td class="col-faktur-id">{{ $item->no_faktur }}</td>
            ...
            <div class="pagination-wrapper">
                <div class="pagination-btns">
                    @if ($transaksiPembelian->onFirstPage() || $transaksiPembelian->isEmpty())
                        <button class="btn-page disabled" disabled>Sebelumnya</button>
                    @else
                        <a href="{{ $transaksiPembelian->previousPageUrl() }}" class="btn-page">Sebelumnya</a>
                    @endif

                    @if ($transaksiPembelian->hasMorePages())
                        <a href="{{ $transaksiPembelian->nextPageUrl() }}" class="btn-page">Berikutnya</a>
                    @else
    ...
    <script>
        ...
        if (filterForm) {
            filterForm.addEventListener('submit', function(e) {
                ...
                const valMulai = document.getElementById('tanggal_mulai').value;
                const valAkhir = document.getElementById('tanggal_akhir').value;
        ...
        document.querySelectorAll('.nav-link, .report-tab').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                if (href && href !== '#' && !this.classList.contains('active') &&
                    !this.classList.contains('active-pembelian') &&
                    !this.classList.contains('active-penjualan')) {
                    e.preventDefault();
                    triggerLoading(href);
                }
            });
